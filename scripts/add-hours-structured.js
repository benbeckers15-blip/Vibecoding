/**
 * add-hours-structured.js
 *
 * Reads every winery document from Firestore, parses the existing `hours`
 ...
 *       node scripts/add-hours-structured.js
 *
...
async function main() {
  ...
  console.log(DRY_RUN
    ...
    : '✏️   LIVE RUN — writing hoursStructured to Firestore');
  console.log(`${'─'.repeat(60)}\n`);

  const snap = await db.collection('wineries').get();
  console.log(`Found ${snap.size} winery documents.\n`);
  ...
  snap.forEach(docSnap => {
    ...
    if (!FORCE_OVERWRITE && Array.isArray(data.hoursStructured) && data.hoursStructured.length > 0) {
      console.log(`  ⏭️  ${id} — already has hoursStructured, skipping`);
      skipped++;
      return;
    }
    ...
    console.log(`  ✅  ${id}`);
    console.log(`       hours (original): ${JSON.stringify(data.hours)}`);
