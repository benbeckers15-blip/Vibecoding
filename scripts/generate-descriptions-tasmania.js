// scripts/generate-descriptions-tasmania.js
//
// Generates editorial winery descriptions + pull quotes for all Tasmania wineries.
// Replaces any existing description/pullQuote fields on Firestore winery documents.
//
// Mirrors the pattern in generate-descriptions.js (Margaret River), but targets the
// Tasmania Firebase project via service-account-tasmania.json and writes review
// outputs to scripts/review/tasmania/ so Tasmania runs never mix with MR runs.
//
// USAGE:
//   node scripts/generate-descriptions-tasmania.js                         ← generate & save review files
//   node scripts/generate-descriptions-tasmania.js --missing-only         ← only wineries with no description in Firestore
//   node scripts/generate-descriptions-tasmania.js --limit 3              ← process first 3 wineries only
//   node scripts/generate-descriptions-tasmania.js --winery "Pooley"      ← single winery by name
//   node scripts/generate-descriptions-tasmania.js --upload               ← upload latest review file to Firestore
//   node scripts/generate-descriptions-tasmania.js --reset                ← clear progress file, start fresh
//
// FLOW (generate):
//   1. Fetch all wineries from the Tasmania Firestore
...
//   5. Save all results to scripts/review/tasmania/ for human approval — nothing touches Firestore yet
//
// FLOW (upload):
//   1. Read the latest review JSON from scripts/review/tasmania/
//   2. Batch-upload description + pullQuote to each Firestore winery document
//
// REQUIREMENTS:
//   - ANTHROPIC_API_KEY environment variable must be set
//   - service-account-tasmania.json must be present in the project root
...
const REVIEW_DIR        = path.join(__dirname, 'review', 'tasmania');
const PROGRESS_FILE     = path.join(__dirname, 'descriptions-progress-tasmania.json');
...
async function researchWithWebSearch(wineryName) {
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Search for information about ${wineryName} winery in Tasmania, Australia. Find facts about: their founding story and year, winemaking philosophy and style, signature or notable wines, any awards or critical recognition, what makes them distinctive among Tasmania producers, and what visitors can expect on-site. Check wine review sites, tourism directories, regional press, and wine publications. Give me a factual summary of what you find.`,
      }],
    });
...
function buildPrompt(winery, sourceContent, sourceType) {
  ...
  const sourceLabel = {
    ...
    general: 'No website content available — use your general knowledge of this winery and the Tasmania region',
  ...
  return `Write an editorial entry for ${winery.name}, a winery in the Tasmania wine region of Australia.
${facts.length > 0 ? `\nKnown facts:\n${facts.map(f => `- ${f}`).join('\n')}` : ''}
...
DESCRIPTION — a JSON array of exactly 3 strings (paragraphs). Combined word count: 300–400 words.
  ...
  Paragraph 3 — THE STORY: Founding history, what gives this winery its particular identity in Tasmania. Notable milestones, the people behind it, what makes it significant in the region. Close with one concrete reason a visitor should make the trip.
...
function saveReviewFiles(results) {
  ...
  const md = [
    `# Winery Descriptions — Tasmania — Review`,
    ``,
    ...
    `\`\`\``,
    `node scripts/generate-descriptions-tasmania.js --upload`,
    `\`\`\``,
    ``,
    `---`,
    ``,
...
async function runGenerate({ limit, wineryFilter, missingOnly }) {
  console.log('\n🍷  Winery Description Generator — Tasmania');
  console.log('─────────────────────────────────────');
  if (limit)       console.log(`📏  Limit: first ${limit} wineries`);
  if (wineryFilter) console.log(`🔍  Filter: "${wineryFilter}"`);
  if (missingOnly) console.log(`🎯  Mode: missing descriptions only (skipping wineries that already have one)`);
  console.log('');
  ...
  let wineries = snapshot.docs
    ...
    .filter(w => w.name && !w.id.startsWith('_placeholder')) // skip placeholder docs
  ...
  // --missing-only: filter to wineries that have no description in Firestore.
  // More reliable than the progress file — checks the actual data, not local state.
  if (missingOnly) {
    const before = wineries.length;
    wineries = wineries.filter(w => !Array.isArray(w.description) || w.description.length === 0);
    console.log(`📊  ${wineries.length} missing descriptions (${before - wineries.length} already have one — skipped)\n`);
  }
  ...
  if (results.length > 0) {
    ...
    console.log('    node scripts/generate-descriptions-tasmania.js --upload\n');
  } else {
...
async function runUpload() {
  console.log('\n🚀  Uploading descriptions to Tasmania Firestore...');
  console.log('──────────────────────────────────────────────');

  const reviewFile = getLatestReviewJson();
  if (!reviewFile) {
    console.error('❌  No review file found in scripts/review/tasmania/');
  ...
  if (uploaded > 0) console.log('\n🎉  Descriptions are live in Tasmania Firestore!\n');
}
...
async function main() {
  ...
  await runGenerate({ limit, wineryFilter, missingOnly });
  await app.delete();
  process.exit(0);
}
